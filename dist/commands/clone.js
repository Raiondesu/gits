...
exports.default = {
    ...
    action: function (_repo, _submodules) {
        return __awaiter(this, void 0, void 0, function () {
            var submodules, repo, submodulesStr, last, others, repoName;
            return __generator(this, function (_a) {
                submodules = _submodules.map(function (s) { return chalk_1.default.blueBright(s); });
                repo = chalk_1.default.yellow(_repo);
                submodulesStr = '';
                if (submodules) {
                    if (submodules.length > 1) {
                        last = submodules[submodules.length - 1];
                        others = submodules.slice(0, submodules.length - 1).join(', ');
                ...
                repoName = (_repo.match(/([a-z0-9-]+)\.git$/) || [])[1];
                if (!repoName) {
                    console.error('Invalid git repo url!');
                    process.exit(1);
                }
                console.log("Cloning " + submodulesStr + "\nfrom " + repo + "\ninto " + process.cwd().replace(/\\/g, '/') + "/" + repoName + "...");
                child_process_1.spawnSync('git', ['clone', _repo, repoName], { stdio: 'inherit' });
                child_process_1.spawnSync('git', ['submodule', 'update', '--init', '--'].concat(_submodules), {
                    stdio: 'inherit',
